...
.. admonition:: ``ECDSA_Signature_Operation::raw_sign()``
   ...
   **Steps:**
   ...
   4. Compute :math:`k'^{-1} = (b_i \cdot k)^{-1}` using the blinding
      value :math:`b_i`. The inversion either uses Fermat's little theorem or a
      curve-specific addition chain if available.

   4. Compute :math:`u=(d \cdot b) \cdot r + m \cdot b`. The braces indicate the order of evaluation.

   6. Compute :math:`s = k'^{-1} \cdot u` which amounts to :math:`(b \cdot k )^{-1} \cdot (d \cdot r + e) \cdot b = k^{-1} \cdot (d \cdot r + m)`.
      The inversion is done wit a variable time exponentiation.

   7. Square the blinding values :math:`b_i` to
      obtain new blinding values :math:`b_{i+1}` which replaces :math:`b_i` in 
