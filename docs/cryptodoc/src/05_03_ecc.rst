...
:srcref:`[src/lib/pubkey/ec_group]/ec_group.cpp`). Previous
...
.. table:: Private Elliptic Curve Data Structures
   ...
   | (:srcref:`code <[src/lib/math/pcurves/pcurves_impl]/pcurves_impl.h>`)                               |                                                                                                                                                                                                                                                      |
   +-----------------------------------------------------------------------------------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
:srcref:`[src/lib/math/pcurves/pcurves_impl]/pcurves_impl.h`) and
projective-projective additions (see
:srcref:`[src/lib/math/pcurves/pcurves_impl]/pcurves_impl.h`). Both
...
:srcref:`[src/lib/math/pcurves/pcurves_impl]/pcurves_impl.h`).
Additionally, iterated point doubling is provided (see
:srcref:`[src/lib/math/pcurves/pcurves_impl]/pcurves_impl.h`) with
...
in :srcref:`src/lib/pubkey/ecc_key/ecc_key.cpp`. To generate a
...
:srcref:`[src/lib/pubkey/ecdh]/ecdh.cpp`. The algorithm receives
...
.. admonition:: ``ECDSA_Signature_Operation::raw_sign()``
   ...
   **Steps:**
   ...
   1. :math:`e = H` and truncate :math:`e` to be at most :math:`bit_length(n)` bits long.
   ...
   4. Compute :math:`k'^{-1} = (b_i \cdot k)^{-1}` using the blinding
   ...
   4. Compute :math:`u=(d \cdot b) \cdot r + e \cdot b`. The braces indicate the order of evaluation.

   6. Compute :math:`s = k'^{-1} \cdot u` which amounts to :math:`(b \cdot k )^{-1} \cdot (d \cdot r + e) \cdot b = k^{-1} \cdot (d \cdot r + m)`.

   7. Square the blinding value :math:`b_i` to
...
The signature generation algorithm works as follows (see
:srcref:`[src/lib/pubkey/eckcdsa]/eckcdsa.cpp`):
...
The signature generation algorithm works as follows (see
:srcref:`[src/lib/pubkey/ecgdsa]/ecgdsa.cpp`):
