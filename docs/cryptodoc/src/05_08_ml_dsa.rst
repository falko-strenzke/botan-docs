...
Botan uses an incompatible PKCS#8 format: The current state of the ML-DSA implementation of Botan produces and consumes PKCS#8 keys that incompatible with the definition of the PKCS#8 format for ML-DSA in RFC 9881. Botan embeds the raw seed in the PKCS#8 structure as the key, whereas RFC 9881 defines `three different private key choices <https://www.rfc-editor.org/rfc/rfc9881.html#name-private-key-format>`, none of which coincides with the format generated and consumed by Botan.
